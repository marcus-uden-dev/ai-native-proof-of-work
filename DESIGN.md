...
- The Job-agent story: a Job-agent-only, evidence-backed product playback. Static HTML is the canonical fallback: a stable left-to-right desktop chapter strip links to complete chapter records, and each record keeps its date, public evidence edition, product change, mechanism, decision annotation, recruiter capability signals, next question, evidence links, and reviewed screenshot together. JavaScript progressively enhances the same records with Previous, Play from origin, Pause, Current state, and slider controls. Play always starts at the public origin; reduced-motion mode requires manual advance. At a historical chapter, later chapter markers are unavailable. On mobile, retain the vertical chronological trail. Do not present public proof-capture dates as private build dates. Do not use force-directed layouts, decorative graph effects, or colour-only meaning.
