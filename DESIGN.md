...
- Decision log: a stacked, single-column list of typed, dated entries (date, primary-capability pills, subdued mechanism metadata, one-line why, one-line what-it-shows), rendered directly in static HTML (never client-fetched) so content is present in the raw page a non-JS-executing reader or agent receives. Reuses `.limitations-list--stacked`'s bordered-stack pattern rather than a card grid, to read as a scannable log, not a case study. `site/evidence/decision-log.json` carries the structured history; `site/evidence/taxonomy.json` is the source of truth for capabilities, mechanisms, role lenses, and migrations. Renders an explicit "no entries yet" state when empty.
